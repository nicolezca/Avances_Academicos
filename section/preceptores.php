...
$preceptores = array(); // Creamos un arreglo para almacenar los datos de los preceptores

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $idPreceptores[] = $row['id'];
        $nombres[] = $row['nombre'];
...
<body>
    ...
    <?php include('../partials/sesionCerrar.php');?>
        ...
        <?php if (isset($idPreceptores) && count($idPreceptores) > 0) : ?>
            <table class="tabla">
                <thead>
                ...
                <tbody>
                    <?php foreach ($idPreceptores as $key => $id) : ?>
                        <tr>
                            <td><?php echo $id; ?></td>
